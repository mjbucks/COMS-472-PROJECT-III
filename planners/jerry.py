...
class PlannerAgent:
    def __init__(self):
        ...
        # Pre-compute common distance calculations
        self.distance_cache = {}
    ...
    def get_state_key(self, current, pursued, pursuer):
        # Enhanced state representation with more granular information
        def discretize(pos, ref):
            dx = (pos[0] - ref[0]) // 3  # More granular grid
            dy = (pos[1] - ref[1]) // 3
            return (dx, dy)
        
        # Use current position as reference point
        pursued_rel = discretize(pursued, current)
        pursuer_rel = discretize(pursuer, current)
        
        # Add distance information to state
        ...
        # Add uncertainty level to state
    ...
    def calculate_reward(self, current, pursued, pursuer):
        # Enhanced reward function with better balance between catching and safety
        dist_to_pursued = abs(current[0] - pursued[0]) + abs(current[1] - pursued[1])
        dist_to_pursuer = abs(current[0] - pursuer[0]) + abs(current[1] - pursuer[1])
        
        # Immediate win/loss rewards
        if dist_to_pursued == 0:
            return 200.0  # Higher reward for winning
        if dist_to_pursuer == 0:
            return -200.0  # Higher penalty for losing
        ...
        if dist_to_pursuer <= 1:
            ...
            safety_reward = 0.0   # Neutral when far enough
        
        # Dynamic weighting based on relative distances
        ...
        # Adjust reward based on uncertainty
        reward *= (1.0 - self.uncertainty * 0.5)
        
        return reward
    
    def choose_action(self, state, legal_actions, epsilon):
        # Enhanced epsilon-greedy with uncertainty consideration
